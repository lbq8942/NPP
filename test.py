...
class TPPCDF(nn.Module):
    ...
    def __init__(
        ...
        self.rnn = getattr(nn, rnn_type)(input_size=num_marks+1, hidden_size=self.context_size, batch_first=True)
        self.timepara=TimePara(self.context_size,self.context_size,self.flownum,self.num_marks)
        self.attn=Attention(args,self.num_features, self.context_size,num_marks=num_marks)
    ...
    def get_features(self, batch):
        ...
        q=self.mark_embedding.weight
        k = self.mark_embedding(batch.marks)  
        return q,k,features  

    def get_context(self, q,k,features,remove_last: bool = True) -> torch.Tensor:
        """
        ...
        """
        context=self.attn(q,k)
        features=torch.cat([context,features],dim=-1)
    ...
    def log_prob(self,batch):  
        ...
        q,k,features = self.get_features(batch)  
        context = self.get_context(q,k,features)  
        ...
        time_pred=0
        return tot_nll, mark_pred,time_pred
